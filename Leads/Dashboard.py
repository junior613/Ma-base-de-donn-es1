    ...
    if uploaded_files:
        ...
        try:
            ...
            if not fichier_selectionne:
                fichier_selectionne = [f for f in tous_fichiers if "leads.xlsx" == os.path.basename(f).lower()]
            if not fichier_selectionne:
            ...
            if fichier_selectionne:
                ...
                nom_fichier_trouve = os.path.basename(fichier_selectionne[0])
                st.sidebar.success(f"Fichier chargé : {nom_fichier_trouve}")
            else:
